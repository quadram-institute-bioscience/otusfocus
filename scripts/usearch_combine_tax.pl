...
die "Missing input file.\n" unless (defined $file);
    ...
    while (my $line = readline($I) ) {
        ...
        my $tax = join(';', taxonomy_to_list($data));
        $taxonomy{$otu_name} = $tax;


    }

## PARSE OTUTABLE /print
open my $TAB, '<', "$opt_input_table" || die " FATAL ERROR: Unable to read input otutab $opt_input_table\n";
my $otutab_lines = 0;
while (my $line = readline($TAB)) {
    chomp($line);
    $otutab_lines++;

    if ($otutab_lines == 1) {
        my @fields = split /\t/, $line;
        say STDERR "$#fields samples found\n";
        $fields[0] = $opt_otu_header;
        push(@fields, 'taxonomy');
        say join("\t", @fields);
    } else {
        my ($otu) = split /\t/, $line;
        say "$line\t$taxonomy{$otu}";
    }
}
...
sub usage {
    ...
    $self [options] -t TAXONOMY -i INPUT.TAB > OUTPUT.TAB
